...
ExpressEngine.prototype.beginBootstrap = function(meanioinstance, database) {
  ...
  if (config.https && config.https.port) {
    var httpsOptions = JSON.parse(JSON.stringify(config.https));
    httpsOptions['key'] = fs.readFileSync(config.https.ssl.key);
    httpsOptions['cert'] = fs.readFileSync(config.https.ssl.cert);
    httpsOptions['ca'] = fs.readFileSync(config.https.ssl.ca);
